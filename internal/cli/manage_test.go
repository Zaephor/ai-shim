...
func TestBackupProfile_Success(t *testing.T) {
	root := t.TempDir()
	layout := storage.NewLayout(root)
	profileDir, err := layout.ProfileHome("test")
	require.NoError(t, err)
...
func TestDiskUsage_PerProfile(t *testing.T) {
	...
	for _, profile := range []string{"work", "personal"} {
		home, err := layout.ProfileHome(profile)
		require.NoError(t, err)
...
func TestAgentVersions(t *testing.T) {
	...
	agentBinDir, err := layout.AgentBin("claude-code")
	require.NoError(t, err)
...
func TestAgentVersions_WithBinary(t *testing.T) {
	...
	binDir, err := layout.AgentBin("claude-code")
	require.NoError(t, err)
...
func TestReinstall_ClearsCacheMarkers(t *testing.T) {
	...
	cacheDir, err := layout.AgentCache("claude-code")
	require.NoError(t, err)
